...
**The Symmetry Problem: queries and documents are structurally different (short/interrogative vs. long/expository), so they form separate clusters in embedding space, causing retrieval to miss relevant documents.**

**Root cause:** Standard embedding models are trained symmetrically (text-to-text), but RAG is asymmetric (question-to-answer).

**Solutions:**
- **HyDE:** Transform queries into hypothetical answers before searching
- **HyPE:** Generate multiple hypothetical passages
- **Asymmetric embeddings:** Use different encoders for queries vs. documents

**Why it matters:** This is the single biggest cause of retrieval failure in naive RAG.
...
When you embed a large corpus of queries and a large corpus of document passages using a **symmetric embedding model** (trained on general text similarity), they do not intermingle freely. Instead, they tend to form distinct clusters. The severity of this separation varies by model -- modern instruction-tuned embedding models (like `bge-*` and `gte-*` series) reduce but do not eliminate this clustering effect.
...
This is the symmetry problem: effective retrieval with **symmetric embedding models** requires symmetric comparison (question-to-question or answer-to-answer), but naive RAG performs an asymmetric comparison (question-to-answer) using a model not trained for it.
