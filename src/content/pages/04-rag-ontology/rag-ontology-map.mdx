...
**Note on Retrieval classification:** Whether a specific retrieval event is a Process or Accomplishment is debatable. The ongoing activity of retrieval is stative (cumulative), but a single retrieval request-response cycle has a natural endpoint, making it arguably an accomplishment. We classify it as Process here to emphasize its cumulative nature across a system's lifetime.
