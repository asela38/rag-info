...
relatedPages: ["02-retrieval-strategies/symmetry-problem", "03-graph-rag/graphrag", "05-synthesis/decision-guide"]
...
**The Problem Tree maps how Naive RAG fails and which techniques solve each failure mode.**

**Four main branches:**
- **Query-Document Mismatch** → HyDE, HyPE, Asymmetric Embeddings
- **Structural Blindness** → GraphRAG, Ontologies, LightRAG, HippoRAG
- **Keyword Blindness** → Hybrid RAG (sparse + dense)
- **Context Loss** → LightRAG (deduplication), HippoRAG (memory + PPR)

**How to use this:** Start with your failure mode, follow the branch to find relevant solutions.
...
flowchart TD
    ...
    C2 --> C2a["HippoRAG + PPR<br/>(memory model +<br/>subgraph discovery)"]
```
</DiagramBlock>
...
| <CrossReference slug="04-rag-ontology/ontology" /> | Define formal categories for graph elements | Distinguish things (endurants) from processes (perdurants) |
...
| <CrossReference slug="03-graph-rag/hipporag" /> | Memory-inspired architecture + PPR | Hippocampal indexing with PageRank-based subgraph discovery |
...
| <CrossReference slug="03-graph-rag/hipporag" /> | Hippocampal memory model + PPR | Pattern separation, completion, and graph-based context expansion |
...
flowchart LR
    ...
    G --> HIPPO["HippoRAG + PPR<br/>(Branch 4)"]
    HIPPO --> F["Final<br/>Answer"]
```
</DiagramBlock>

For example:
- **HyDE + Hybrid RAG**: Transform the query (Branch 1) then use both sparse and dense retrieval (Branch 3)
- **GraphRAG + HippoRAG/PPR**: Build a knowledge graph (Branch 2) and use PageRank to find relevant subgraphs (Branch 4)
...
3. **Finish with Synthesis** — <CrossReference slug="05-synthesis/progressive-map" />
