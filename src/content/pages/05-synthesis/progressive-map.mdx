...
relatedPages: ["05-synthesis/indexing-comparison", "05-synthesis/decision-guide"]
prev: "04-rag-ontology/foundational-ontologies"
next: "05-synthesis/indexing-comparison"
...
**The Progressive Map shows how RAG techniques build incrementally from simple to complex, each adding capabilities to solve specific failures.**

**The layers:**
- **Layer 0:** Naive RAG (baseline)
- **Layer 1:** Query transformation (HyDE, HyPE) + Hybrid search
- **Layer 2:** Graph-based indexing (GraphRAG, LightRAG)
- **Layer 3:** Advanced graph retrieval (HippoRAG + PPR)

**Key insight:** You don't need the most complex approach. Start simple, add complexity only where naive RAG fails for your use case.

**Use this page to:** Understand the progression and see where each technique fits in the overall landscape.
...
|-----------|-----------------|------|
| <CrossReference slug="02-retrieval-strategies/hyde" /> | 1 extra LLM call per query | Higher query latency, potential hallucination in hypothesis |
| <CrossReference slug="02-retrieval-strategies/hype" /> | N LLM calls per chunk (index time only) | Higher indexing cost, zero query latency overhead |
...
| <CrossReference slug="04-rag-ontology/ontology" /> | Formal categorization (endurants vs. perdurants) | Conceptual — guides graph design |
...
|-----------|-------------------|
| <CrossReference slug="03-graph-rag/hipporag" /> | Memory-inspired pattern separation and completion via Personalized PageRank |
...
**You don't need every layer.** Most real-world systems use a subset. The <CrossReference slug="05-synthesis/decision-guide" /> helps you pick which layers to adopt based on your requirements.
...
See how different approaches handle indexing in <CrossReference slug="05-synthesis/indexing-comparison" />, then use the <CrossReference slug="05-synthesis/decision-guide" /> to choose the right combination for your use case.
